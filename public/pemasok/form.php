	...
	<div class="modal fade" id="confirmation-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog" role="document">
			...
			<div class="modal-body">
				Apakah Anda yakin untuk menghapus pemasok ini?
			</div>
...
<script>
	$(document).ready(function() {
		<?php if ($_POST['submit'] == 'update') { ?>
			$.ajax({
				...
				data: {'IdPemasok': <?php echo $_POST['idpemasok']; ?>, 'submit': 'find'},
				dataType: 'json',
	...
	$('#pemasok-form').on('submit', function(e) {
		e.preventDefault()
		...
		$.ajax({
			...
			data: values,
			success: function() {
				window.location.href = "<?php echo url('pemasok/daftar.php'); ?>"
			},
	...
	<?php if ($_POST['submit'] == 'update') { ?>
		...
		$('#confirm-delete').on('click', function(e) {
			...
			$.ajax({
				type: 'POST',
				url: "<?php echo url('action/pemasok.php'); ?>",
				data: {'idpemasok': <?php echo $_POST['idpemasok']; ?>, 'submit': 'delete'},
				success: function() {
					window.location.href = "<?php echo url('pemasok/daftar.php'); ?>"
				}
			})
	...
	function fillForm(values) {
		$('#idpemasok').val(values.IdPemasok)
		$('#pemasok').val(values.NamaPemasok)
		$('#alamat').val(values.AlamatPemasok)
